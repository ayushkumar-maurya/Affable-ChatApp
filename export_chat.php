...
$sql1 .= "AS userClrCount FROM chats_count WHERE user1id IN (:sid, :rid) AND user2id IN (:sid, :rid)";
...
$sql2 = "SELECT senderid, recieverid, message, date_time FROM messages WHERE senderid IN (:sid, :rid) AND recieverid IN (:sid, :rid) ORDER BY date_time LIMIT $fromMsgCnt, $remMsgCnt";
